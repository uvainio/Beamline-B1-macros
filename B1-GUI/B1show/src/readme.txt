 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_B1show.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		...
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* B1show.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* B1show (application created by MATLAB Compiler)
		* run_B1show.sh (shell script that can be run 
	...
	All Platforms:            
		* B1show.dll (shared library, file extension 
		  varies by platform)
		* B1show.h (library header file)
		* B1show.ctf (Component Technology File archive,
	...
	* B1show.xml (documentation files)
	* B1show.pdb (program database file - if 
	  DEBUG option selected)
	* B1show.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* B1show.jar (Java class files and compiled M-code)
