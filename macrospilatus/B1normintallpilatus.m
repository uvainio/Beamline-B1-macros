function B1normintallpilatus(fsn1,thicksfile,sens,errorsens,mask,energymeas,energycalib,distminus,pri,mythendistance,mythenpixelshift,detshift,fluorcorr,refname,orig)
  ...
  %if(emptys(m+1,1) > fsn1found(emptys(m+1,2)-1)) % Process only if next file from empty is not empty
   if(emptys(m+1,2)>(emptys(m,2)+1)) % Process only if next file from empty is not empty, edited 1.7.2012 UV
      if(nargin>14) % 29.6.2011. AW. fixed 13->12
          ...
          B1normintpilatus1(fsn1found(emptys(m,2):(emptys(m+1,2)-1)),thicksfile,sens,errorsens,mask,energymeas,energycalib,distminus,pri,detshift,fluorcorr,refname);
      end;
...
if(nargin>14) % 29.6.2011. AW. fixed 13->12
    B1normintpilatus1(fsn1found(emptys(end,2):end),thicksfile,sens,errorsens,mask,energymeas,energycalib,distminus,pri,detshift,fluorcorr,refname,orig);
else
    B1normintpilatus1(fsn1found(emptys(end,2):end),thicksfile,sens,errorsens,mask,energymeas,energycalib,distminus,pri,detshift,fluorcorr,refname);
end;
% Read in calibrated energy
