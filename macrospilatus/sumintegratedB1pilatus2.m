function datasum = sumintegratedB1pilatus2(data,param,samplename,samplename2)
   ...
   for(l = 1:length(temperatures))
      for(m = 1:length(dist))
          ...
          for(k = 1:sd(2)) % first sum
             if((strcmp(param(k).Title,samplename) || strcmp(param(k).Title,samplename2)) & dist(m)/param(k).Dist > 0.95 & dist(m)/param(k).Dist < 1.05 & round(param(k).EnergyCalibrated) == round(energies(h))) % & temperatures(l)==round(param(k).Temperature))
               param(k).FSN
