...
if(nargin==3) % don't show titles if dontshowtitles is given
  disp(sprintf('FSN\tTime\tEnergy\tDist\tPos\tTransm\tT (C)\tRot Y\tTitle\t\t\tDate'))
end;
if(numel(sizeA)>1) % If more than one matrix
  for(n = 1:sizeA(2))
    disp(sprintf('%d\t%d\t%.1f\t%d\t%.2f\t%.4f\t%.f\t%.1f\t%s\t%s',getfield(header(n),'FSN'),round(getfield(header(n),'MeasTime')),getfield(header(n),'Energy'),...
    getfield(header(n),'Dist'),getfield(header(n),'PosSample'),getfield(header(n),'Transm'),...
    header(n).Temperature,header(n).Rot2,getfield(header(n),'Title'),sprintf('%d.%d.%d %d:%02d',header(n).Day,...
    header(n).Month,header(n).Year,header(n).Hour,header(n).Minutes)));
  end;
else
    disp(sprintf('%d\t%d\t%.1f\t%d\t%.2f\t%.4f\t%.f\t%.1f\t\t\t%s\t%s',getfield(header,'FSN'),round(getfield(header,'MeasTime')),getfield(header,'Energy'),...
    getfield(header,'Dist'),getfield(header,'PosSample'),getfield(header,'Transm'),...
    header.Temperature,header.Rot2,getfield(header,'Title'),sprintf('%d.%d.%d %d:%02d',header.Day,...
