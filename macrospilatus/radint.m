...
warning('Executing the script-version of radint! This can be slower than the mex version. Please build radint2.c by: "mex -v -DRADINT radint2.c -output radint" if you want to use the mex version');
if size(data)~=size(dataerror) or size(data)~=size(mask)
    error('Sizes of data, dataerror and mask should be equal.')
if length(res)==1
    res=[res, res];
if length(res)>2
...
D=sqrt(((res(1)/nsubdivx)*(X-bcx)).^2+((res(2)/nsubdivy)*(Y-bcy))^2);
disp('done')
